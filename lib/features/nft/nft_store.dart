...
abstract class _NftStore with Store {
  ...
  @computed
  ObservableList<NftFamilyItem> get nftAssets =>
      ObservableList.of(_tokenStore.nftCollectibles.map((nftCollectible) {
        ...
        nftCollectible.groupIdPayloadDict.forEach((groupId, payload) {
          ...
          logger.e('${nftCollectible.groupIdQuantityDict[groupId] ?? 0}');
          collectibles.add(
            NftCollectibleItem(nftCollectible.groupIdQuantityDict[groupId] ?? 0,
                getNftPayloadType(payloadTypeName), title, url, payloadContent),
          );
        });
