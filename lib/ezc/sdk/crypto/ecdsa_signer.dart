...
class EzcECDSASigner implements Signer {
  ...
  static final FactoryConfig factoryConfig = DynamicFactoryConfig.regex(
    ...
    return () {
      ...
      return EzcECDSASigner(underlyingDigest, mac);
    };
  });
  ...
  EzcECDSASigner([this._digest, this._kMac]);
  ...
  Signature generateSignature(Uint8List message) {
    ...
    return EzcECSignature(r, s, recoveryParam);
  }

  @override
  bool verifySignature(Uint8List message, covariant EzcECSignature signature) {
    message = _hashMessageIfNeeded(message);
...
class EzcECSignature implements Signature {
  ...
  EzcECSignature(this.r, this.s, this.recoveryId);

  factory EzcECSignature.fromSignature(Uint8List signature) {
    ...
    return EzcECSignature(r, s, recoveryId);
  }
  ...
  ///
  EzcECSignature normalize(ECDomainParameters curveParams) {
    if (isNormalized(curveParams)) {
      return this;
    }
    return EzcECSignature(r, curveParams.n - s, recoveryId);
  }
  ...
  @override
  bool operator ==(other) {
    if (other is! EzcECSignature) return false;
